'''
Cálculo do vetor rotação utilizando-se de vanishing points. O modo de
utilização está descrito no __main__
'''
...
class Orientation():
    ...
    def findVanishingPoint(self, dst):
        ...
        p0 = [dst[0][0][0], dst[0][0][1], 1]
        p1 = [dst[0][1][0], dst[0][1][1], 1]
        p2 = [dst[0][2][0], dst[0][2][1], 1]
        p3 = [dst[0][3][0], dst[0][3][1], 1]
...
if __name__ == '__main__':
    ...
    myFinder = Finder(template)

    img = calibrateImagem(img)
    dst, img2 = myFinder.find_marker(img)
